...
PRESET_URL="$(cat "$ROOT_DIR/configs/shadcn-preset.txt")"

for APP_DIR in "$ROOT_DIR/apps/web" "$ROOT_DIR/apps/docs-site"; do
  if [[ ! -d "$APP_DIR" ]]; then
    echo "missing app directory: $APP_DIR" >&2
    exit 1
  fi

  echo "initializing shadcn in $APP_DIR"
  (
    cd "$APP_DIR"
...
CSS
    ...
    bun remove lucide-react radix-ui || true
  )
done
