...
use oore_contract::{
    ...
    TransferOwnerRequest, TransferOwnerResponse, UpdateUserRoleRequest, UpdateUserRoleResponse,
...
/// `POST /v1/users/transfer-owner` — transfer owner to an existing active user.
pub async fn transfer_owner(
    State(state): State<Arc<AppState>>,
    auth: AuthUser,
    Json(req): Json<TransferOwnerRequest>,
) -> ApiResult<TransferOwnerResponse> {
    auth.require_owner()?;

    let email = req.email.trim().to_lowercase();
    if email.is_empty() || !email.contains('@') || email.len() > 256 {
        return Err(api_err(
            StatusCode::BAD_REQUEST,
            "invalid_email",
            "Invalid email address",
    ...
    .await
    .map_err(|e| {
        error!(error = %e, "failed to fetch current owner");
        api_err(
            StatusCode::INTERNAL_SERVER_ERROR,
            "store_error",
    ...
    .ok_or_else(|| {
        api_err(
            StatusCode::FORBIDDEN,
            "owner_session_stale",
            "Current session is no longer the owner",
        )
    })?;
    let previous_owner = row_to_user(&previous_owner_row);

    let target_row = sqlx::query(
        "SELECT id, email, oidc_subject, display_name, role, status, avatar_url, created_at, updated_at \
    ...
    .await
    .map_err(|e| {
        error!(error = %e, "failed to fetch owner transfer target");
        api_err(
            StatusCode::INTERNAL_SERVER_ERROR,
            "store_error",
    ...
    let target_subject: String = target_row.get("oidc_subject");
    if target_status != "active" {
        return Err(api_err(
            StatusCode::CONFLICT,
            "user_not_active",
            "Owner can only be transferred to an active user",
        ));
    }
    if target_role == "owner" {
        return Ok(Json(TransferOwnerResponse {
            previous_owner: row_to_user(&target_row),
            owner: row_to_user(&target_row),
        }));
    }

    let now = now_unix();
    let mut tx = pool.begin().await.map_err(|e| {
        error!(error = %e, "failed to begin owner transfer transaction");
        api_err(
            StatusCode::INTERNAL_SERVER_ERROR,
            "store_error",
    ...
    sqlx::query("UPDATE users SET role = 'admin', updated_at = ?1 WHERE role = 'owner'")
        .bind(now)
        .execute(&mut *tx)
        .await
        .map_err(|e| {
            error!(error = %e, "failed to demote previous owner");
            api_err(
                StatusCode::INTERNAL_SERVER_ERROR,
                "store_error",
                "Failed to transfer owner",
    ...
    sqlx::query(
        "UPDATE users SET role = 'owner', status = 'active', updated_at = ?1 WHERE id = ?2",
    )
    .bind(now)
    .bind(&target_id)
    .execute(&mut *tx)
    .await
    .map_err(|e| {
        error!(error = %e, "failed to promote new owner");
        api_err(
            StatusCode::INTERNAL_SERVER_ERROR,
            "store_error",
            "Failed to transfer owner",
    ...
    sqlx::query(
        "UPDATE setup_state SET owner_email = ?1, owner_oidc_subject = ?2, owner_created_at = ?3, updated_at = ?3 WHERE id = 1",
    )
    .bind(&email)
    .bind(&target_subject)
    .bind(now)
    ...
    .await
    .map_err(|e| {
        error!(error = %e, "failed to update setup owner record");
        api_err(
            StatusCode::INTERNAL_SERVER_ERROR,
            "store_error",
    ...
    tx.commit().await.map_err(|e| {
        error!(error = %e, "failed to commit owner transfer");
        api_err(
            StatusCode::INTERNAL_SERVER_ERROR,
            "store_error",
            "Failed to transfer owner",
    ...
    state
        .sessions
        .revoke_user_sessions(&previous_owner.id)
        .await
        .map_err(|e| {
            error!(error = %e, "failed to revoke previous owner sessions");
            api_err(
                StatusCode::INTERNAL_SERVER_ERROR,
                "session_error",
                "Failed to revoke previous owner sessions",
            )
        })?;
    state
        .sessions
        .revoke_user_sessions(&target_id)
        .await
        .map_err(|e| {
            error!(error = %e, "failed to revoke new owner sessions");
            api_err(
                StatusCode::INTERNAL_SERVER_ERROR,
                "session_error",
                "Failed to revoke new owner sessions",
            )
        })?;
    ...
    let previous_owner = fetch_user_by_id(&state, &previous_owner.id).await?;
    let owner = fetch_user_by_id(&state, &target_id).await?;
