...
`oore-web` proxies `/v1/*` and `/healthz` to `OORE_WEB_BACKEND_URL`. If you use trusted-proxy authentication, preserve the identity email header and optional `X-Oore-Trusted-Proxy-Secret` through every proxy hop.
