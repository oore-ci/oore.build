...
If no eligible builds are queued, the endpoint returns `204 No Content`. Blocked builds remain queued, and their ordinary user-facing build responses include `runner_policy_block_reason`.
