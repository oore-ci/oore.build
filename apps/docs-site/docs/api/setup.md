...
For the full setup flow, see the [Setup Wizard](/features/setup-wizard) feature documentation.
...
**State requirement**: Available in all states
...
Verify the one-time bootstrap token generated by [`oore setup open`](/cli/setup#setup-open). On success, consumes the token and returns a setup session token.
...
Initiate an OIDC authorization code flow to verify the owner's identity. Returns an authorization URL that the user should be redirected to.
