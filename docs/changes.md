...
- **Stable-release UI and frontend engineering hardening**:
  ...
  - QA app selection now uses one compact, searchable shadcn Combobox directly in the build activity header instead of a persistent app rail or separate picker dialog. The selected app appears once, filtering preserves the workspace, and unmatched searches show “No matching apps” inside the selector.
