...
- **Stable-release UI and frontend engineering hardening**:
  ...
  - QA app selection now uses one full-width, searchable shadcn Combobox directly in the build activity header instead of a persistent app rail or separate picker dialog. The selector shows the selected app’s logo and proper object label without a redundant build count, filtering preserves the workspace, unmatched searches show “No matching apps” inside the selector, and the activity list has no outer card boundary.
