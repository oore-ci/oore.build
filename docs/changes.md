...
- **Stable-release UI and frontend engineering hardening**:
  ...
  - QA app selection now uses one compact, searchable shadcn Command picker instead of a persistent app rail. Filtering stays inside the picker, preserves input focus and the selected app, and shows “No matching apps” without replacing the workspace with the no-access empty state.
