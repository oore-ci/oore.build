...
- **Stable-release UI and frontend engineering hardening**:
  ...
  - Live UI review simplified operator artifact rows to one consistent install/download action plus sharing, removed the transient log jump control, standardized Project Access user selection on the shared Combobox, stabilized the Builds filter toolbar with its reserved Clear action before the selectors, aligned Users summaries and invite actions with other settings inventories, restored human-readable retention labels, prioritized runtime facts and flattened nested External Access boundaries in Preferences, replaced Audit Log date inputs with the official shadcn Calendar range picker, reset Badge to the shadcn registry implementation while keeping noninteractive badges off the primary action color, and made Sources inventory-first with a single provider picker once a connection exists.
