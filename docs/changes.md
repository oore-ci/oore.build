...
- **Stable-release UI and frontend engineering hardening**:
  ...
  - Live UI review simplified operator artifact rows to one consistent install/download action plus sharing, removed the transient log jump control, standardized Project Access user selection on the shared Combobox, stabilized the Builds filter toolbar, restored human-readable retention labels, flattened nested External Access boundaries, combined Audit Log dates into one explicit range control, and made Sources inventory-first with a single provider picker once a connection exists.
