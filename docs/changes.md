...
- **Stable-release UI and frontend engineering hardening**:
  ...
  - QA now has a theme toggle in its compact header, and demo mode adds 24 older builds to every demo app so the tester workspace visibly exercises its existing 10-row pagination through the shared shadcn Pagination controls.
