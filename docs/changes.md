...
- **Smaller, truthfully budgeted web startup**:
  ...
  - Operator build detail keeps artifact rows and direct downloads on first paint while loading the share-menu, scoped-link dialog, select, and checkbox machinery on hover, focus, or click. Its base payload falls from 265.29 KiB to 205.44 KiB gzip; the expanded sharing path is 267.59 KiB and remains budgeted at 275 KiB.
  - QA artifact install loads the existing safe Markdown renderer only when a build actually has a changelog. Its base payload falls from 208.42 KiB to 174.99 KiB gzip; the 208.78 KiB changelog-expanded path remains budgeted at 215 KiB with identical Markdown, link, and raw-HTML handling.
  ...
  - The existing dynamically imported `web-vitals` package is reused with no analytics SDK. The full field-metrics path is 168.74 KiB gzip against a 170 KiB post-idle budget while initial JavaScript remains 165.38 KiB.
